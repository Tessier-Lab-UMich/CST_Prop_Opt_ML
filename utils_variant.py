...
def scatter_hist(x, y, ax, ax_histx, ax_histy, target, color):
    # no labels
    ax_histx.tick_params(axis="x", labelbottom=False)
    ax_histy.tick_params(axis="y", labelleft=False)

    # the scatter plot:
    ax.scatter(x, y, s=50, c=color, edgecolors=(0, 0, 0), linewidth = 0.25)
...
def gnb_prob_plot_sins(clf, data_scaled, target, cin_data_scaled, cin_predict, gan_data_scaled, gan_predict, pan_data_scaled, pan_predict):
    ...
    contours = ax.contour(xx, yy, Z, levels=[0.5], linewidths=1,
                               colors=['k'])
    scatter_hist(cin_data_scaled[:,0], cin_data_scaled[:,1], ax, ax_histx, ax_histy, cin_predict, 'blueviolet')
    scatter_hist(gan_data_scaled[:,0], gan_data_scaled[:,1], ax, ax_histx, ax_histy, gan_predict, 'magenta')
    scatter_hist(pan_data_scaled[:,0], pan_data_scaled[:,1], ax, ax_histx, ax_histy, pan_predict, 'orange')
...
def gnb_prob_plot_smp(clf, data_scaled, target, cin_data_scaled, cin_predict, gan_data_scaled, gan_predict, pan_data_scaled, pan_predict):
    xx, yy = np.meshgrid(np.linspace(-0.05, max(data_scaled[:,0])+0.05, 500), np.linspace(-0.05,  max(data_scaled[:,1])+0.05, 500))
    Z = clf.predict_proba(np.vstack((xx.ravel(), yy.ravel())).T)[:, 1]
    Z = Z.reshape(xx.shape)
    fig = plt.figure(figsize = (8,6))
